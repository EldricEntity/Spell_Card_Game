...
import datetime
import oracledb
import contextlib  # Needed for @contextlib.contextmanager
...
# ORACLE_CLIENT_LIB_DIR is no longer explicitly needed since we're implicitly using Thin Mode
    ...
    try:
        ...
        try:
            ...
            if not table_exists:
                ...
                conn.commit()
                print("CARDS table created and populated.", flush=True)
            else:
                cursor.execute("SELECT COUNT(*) FROM CARDS")
                card_count = cursor.fetchone()[0]
                if card_count == 0:
                    print("CARDS table exists but is empty. Populating with initial data.", flush=True)
                    cursor.executemany("""
                        INSERT INTO CARDS (id, name, image_filename, description, type, rarity, default_uses_per_rest)
                        VALUES (:id, :name, :image_filename, :description, :type, :rarity, :default_uses_per_rest)""",
                                       DEFAULT_CARDS
                    ...
                    conn.commit()
                    print("CARDS table populated.", flush=True)
                else:
                    print("CARDS table already contains data. Skipping population.", flush=True)
...
def get_player_deck():
    ...
    try:
        with get_db_connection() as connection:
            with connection.cursor() as cursor:
                # CRITICAL CHANGE: Select deck_json directly as CLOB, without TO_CHAR
                cursor.execute("""
                ...
                if row:
                    ...
                    # CRITICAL CHANGE: Explicitly read the LOB object into a string
